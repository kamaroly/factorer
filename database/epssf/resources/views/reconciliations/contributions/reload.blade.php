...
@section('content')
<a href="{{ route('reconciliations.contributions.download','matching') }}" class="btn bg-green-700 text-green-200 hover:text-green-100">
	Matching
	{{ $inBoth->count() }} 
</a>
<a href="{{ route('reconciliations.contributions.download','not-matching') }}" class="btn bg-red-700 text-red-200"> 
	Not Matching in Amount
	{{ $notMatchingContribution->count() }}
</a>
<a href="{{ route('reconciliations.contributions.download','in-system-not-csv') }}" class="btn bg-gray-700 text-gray-200"class="btn">
	Exists in System but Not In Csv Matching
	{{ $onlyInDb->count() }}
</a>
<a href="{{ route('reconciliations.contributions.download','in-csv-not-system') }}" class="btn bg-indigo-700 text-indigo-200"class="btn">
