jQuery(document).ready(function($) {
    ...
    notification +='<button type="button" class="close" data-dismiss="alert" aria-hidden="true">Ã—</button>';
	...
	$('.loan-select').change(function(event) {
		...
		// Recalculate all details on the page if 
	 ...
	 * @return
	 */
	function calculateGuarantors() {

	 // Show caution warning
	 $('#cautionForm').css('display','none');

 	 var previousLoanBalance = 0;
	 var loanToRepay         = 0;
	 var totalSavings 	     = 0;

	 if ($('#previous_loan_balance').val() != 'undefined' && $('#previous_loan_balance').val() != null) {
			 previousLoanBalance = parseInt($('#previous_loan_balance').val().replace(/,/g,''));	 	
	 }

 	 if ($('#loanToRepay').val() != 'undefined' && $('#loanToRepay').val() != null) {
			 loanToRepay = parseInt($('#loanToRepay').val().replace(/,/g,''));	 	
	 }

 	 if ($('#totalContributions').val() != 'undefined' && $('#totalContributions').val() != null) {
			 totalSavings = parseInt($('#totalContributions').val().replace(/,/g,''));	 	
	 }

	 /// if ($('#operation_type').val() =='special_loan') {
	 /// var olivier =$('#operation_type').val();
        	/// console.log(olivier);
	 ///  $('#js-guarantor-warning').css('display', 'none');
 	  /// $('#cautionForm').css('display','none');
	 ///}
  
	var bondedAmount = totalSavings - ( previousLoanBalance + loanToRepay );

	//  If bondedAmount is Positive, then don't set the Cautionneur
	...
	//this was request of epssf to not ask guarantors for special_loan

	 if (bondedAmount < 0 && $('#operation_type').val() =='special_loan')
         {          
         	//var olivier =$('#operation_type').val();
        	//console.log(olivier);
	 	 // If this person has previous loan, then consider 
	 	 // use new loan amount to repay as the bond
	 	
  	 	/// $('#amount_bonded').val(Math.abs(bondedAmount));
	 	 $('#js-guarantor-warning').css('display', 'none');
 	 	 $('#cautionForm').css('display','none');
	 }
	 
	 // Requested by @olivier to disable this functionality.
	 else if  (bondedAmount < 0) {
	       if (previousLoanBalance > 0 ) {
		 	 bondedAmount    = loanToRepay;	 	 	
	 	 }	  
	 	 	//var olivier =$('#operation_type').val();
	...
	function calculateLoanDetails(){
		...
		//     1200 + (TI*N)
		//
		// Where :Â Â Â I : Interest 
		// Â Â Â Â Â Â Â Â Â Â P : Amount to Repay
		// Â Â Â Â Â Â Â Â Â Â TI: Interest Rate
		// Â Â Â Â Â Â Â Â Â Â N : Montly payment
		// LoanToRepay * (InterestRate*NumberOfInstallment) / 1200 +(InterestRate*NumberOfInstallment)
		///formula have been changed as I= P * TI *N/12 in case of N is between 1 to 12 months
		/// or I= P * TI *N/24 in case of N is between 13to 24 months
		
          if(numberOfInstallment<=12)
          {

		var interests = (loanToRepay * numberOfInstallment*2)/1200;
	      }
	      else
	          {

		var interests = (loanToRepay * numberOfInstallment*4)/2400;
	      }
        ...
        /**requested by epssf to remove interest on special loan */
			if ($('#operation_type').val() =='special_loan') {
		 	interests =  loanToRepay * 0;
		};
	...
	/**
	 * De 1 Ã  12 mois le taux dâ€™intÃ©rÃªt est de 3.4
	Âƒ * De 13 Ã  24 mois le taux dâ€™intÃ©rÃªt est de 3.6
	Âƒ * De 25 Ã  36 mois le taux dâ€™intÃ©rÃªt est de 4.1
	Âƒ * De 37 Ã  48 mois le taux dâ€™intÃ©rÃªts est de 4.3
	 ...
	 */
    function validateLoanToRepay(){

		var rightToLoanAmount = parseInt($('#rightToLoan').val().replace(/,/g,''));

		var loanToRepay       = parseInt($('#loanToRepay').val()); 
      
      // Display warning when wished amount or loan to replay is greater than 
      // Right to loan
      if(loanToRepay > rightToLoanAmount){
		    notification = notification.replace('message','Loan To Repay amount has to be less or equal to Right to loan');
			$('.loan-notifications').html(notification);
			$('#loanToRepay').addClass('has-error');
			return false;
		}
		// For us to reach here
		// All is good ! time to sleep
		$('.loan-notifications').html('');
		$('#loanToRepay').removeClass('has-error');
		return true;
    }
