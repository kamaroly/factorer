...
<caption> {{ trans('report.year_interest_title') }} </caption>
  	 <thead>
  	 	<tr>
			...
			<td>{{ trans('member.transaction_type') }}</td>
	        <td>{{ trans('member.transaction_reason') }}</td>	
			<th>{{ trans('member.status') }}</th>
			<th>{{ trans('member.savings') }}</th>
	  	</tr>
   	 </thead>
 <tbody>
   @each ('reports.member.item_year_interest', $members, 'member', 'members.no-items')
   <tr>
	  	 	<th colspan="7">{{ trans('member.service') }}</th>
